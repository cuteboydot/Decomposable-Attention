    ...
    def _build_graph(self):
        ...
        with tf.device(self.dev):
            ...
            with tf.variable_scope("attention_layer"):
                self.e = tf.tanh(tf.matmul(self.output_enc1, self.output_enc2, transpose_b=True), name="e")
                ...
                self.beta = tf.matmul(self.attention1, self.output_enc2, name="beta")
                ...
                self.alpha = tf.matmul(self.attention2, self.output_enc1, name="alpha")
                print("alpha.get_shape() : %s" % (self.alpha.get_shape()))
                                                                                    
            with tf.variable_scope("comparison_layer"):
                self.aligned1 = tf.concat([self.output_enc1, self.beta], axis=2, name="aligned1")
                self.aligned2 = tf.concat([self.output_enc2, self.alpha], axis=2, name="aligned2")
                
                self.v1 = tf.layers.dense(self.aligned1, self.bi_state_dim*2, activation=tf.nn.relu, name="g1")
                self.v1 = tf.layers.dense(self.v1, self.bi_state_dim*2, activation=tf.nn.relu, name="g2")
                print("v1.get_shape() : %s" % (self.v1.get_shape()))
                
                self.v2 = tf.layers.dense(self.aligned2, self.bi_state_dim*2, activation=tf.nn.relu, name="g1", reuse=True)
                self.v2 = tf.layers.dense(self.v2, self.bi_state_dim*2, activation=tf.nn.relu, name="g2", reuse=True)
            ...
            if self.makedir == True:
                ...
                self.out_dir = os.path.abspath(os.path.join("./model_rnn", timestamp))
